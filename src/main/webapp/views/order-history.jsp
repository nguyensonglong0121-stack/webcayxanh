...
<section class="section">
    <div class="container">
        ...
        <c:if test="${param.deleted == '1'}">
            <div class="alert alert-success">🗑️ Đã xóa đơn hàng thành công!</div>
        </c:if>
                        ...
                        <div style="background:white;border:1px solid var(--sand);border-radius:var(--radius);padding:20px">
                            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
                                ...
                                <c:if test="${o.status == 'cancelled'}">
                                    <div style="margin-top:12px">
                                        <form action="${pageContext.request.contextPath}/orders" method="post"
                                              onsubmit="return confirm('Xóa vĩnh viễn đơn #${o.orderId}? Hành động này không thể hoàn tác.')">
                                            <input type="hidden" name="action"  value="deleteOrder">
                                            <input type="hidden" name="orderId" value="${o.orderId}">
                                            <button type="submit" class="btn btn-danger btn-sm">🗑️ Xóa đơn hàng</button>
                                        </form>
                                    </div>
                                </c:if>
